...
template = (
    ...
    "당신의 임무는 주어진 지문(Paragraph)과 보조 자료(Support)를 바탕으로, 질문(Question)에 가장 적합한 답변을 선택지(Choices)에서 선택하는 것입니다.\n"
    "선택지(Choices)는 총 {len_choice}개이며, 각 선택지(Choices)에는 고유한 선택지 번호(1~{len_choice})가 있습니다. "
